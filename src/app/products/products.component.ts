...
export class ProductsComponent implements OnInit {
  query: string = '';
  constructor(
    public productsRepositoryService: ProductsRepositoryService,
    ...
    this.productsRepositoryService.getProducts(keyword, page, size).subscribe({
      next: (resp) => {
        ...
        let totalPages = Math.floor(
          totalCount / this.appState.productsState.pageSize
        );
        if (totalCount % this.appState.productsState.pageSize !== 0)
  ...
  handleCheckProduct(product: any) {
    this.appState.productsState.status = 'LOADING';
    this.productsRepositoryService.checkProduct(product).subscribe({
      next: (data) => {
        let prods = this.appState.productsState.products.map((p: any) =>
  ...
  handleDeleteProduct(product: any) {
    this.appState.productsState.status = 'LOADING';
    this.productsRepositoryService.deleteProduct(product).subscribe({
      next: (data) => {
        let products = this.appState.productsState.products.filter(
